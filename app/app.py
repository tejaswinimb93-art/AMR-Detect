...
if __name__ == "__main__":
    # Render supplies the web-service port through PORT.
    # Bind to all interfaces so Render can detect the Gradio server.
    port = int(os.environ.get("PORT", "10000"))
    print(f"AMRIVA starting on 0.0.0.0:{port}", flush=True)
    demo.launch(
        server_name="0.0.0.0",
        server_port=port,
        share=False,
